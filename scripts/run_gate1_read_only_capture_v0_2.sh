...
: "${RUN_ID:=gate1a-$(date -u +%Y%m%dT%H%M%SZ)}"
...
if [[ "${EVE_Q_GATE1_PILOT:-0}" != "1" ]]; then
  echo "Set EVE_Q_GATE1_PILOT=1 for an explicit approved Gate 1A run." >&2
  exit 1
fi
...
then
  ...
  echo "Gate 1 v0.2 source capture failed closed." >&2
  exit 1
fi

if ! codex-gate1 replay \
  --bundle "$BUNDLE" \
  --require-fresh
then
  write_rollback "operator_abort"
  echo "Gate 1 v0.2 offline replay failed closed." >&2
  exit 1
fi
...
printf 'Gate 1A read-only v0.2 capture: PASS\n'
printf 'Run ID: %s\n' "$RUN_ID"
printf 'Output: %s\n' "$OUT"
printf 'Snapshot: %s\n' "$BUNDLE"
